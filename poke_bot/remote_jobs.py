...
# Fleet roots and aliases are deployment inputs in the public source release.
# Private development defaults are intentionally not embedded here.
_TRAIN_ROOT = Path(
    os.environ.get("POKEBOT_TRAIN_ROOT", str(Path.cwd()))
).expanduser()
_BERT_ROOT = Path(
    os.environ.get(
...
def _host_aliases(name: str, fallback: str) -> frozenset[str]:
    raw = os.environ.get(name, fallback)
    return frozenset(value.strip() for value in raw.split(",") if value.strip())


_ELMO_HOSTS = _host_aliases("POKEBOT_ELMO_HOSTS", "elmo")
_BERT_HOSTS = _host_aliases("POKEBOT_BERT_HOSTS", "bert")
_BERT_SSH = os.environ.get("POKEBOT_BERT_SSH", "bert")
...
def endpoint_role(host: str) -> str:
    ...
    host_l = str(host).strip().lower()
    if host_l in _ELMO_HOSTS or host_l.startswith("elmo"):
        return "elmo"
...
def endpoint_default_workers(host: str, port: Optional[int] = None) -> int:
    ...
    ``elmo:8765=20,bert.local:8766=10``.
    """
...
def endpoint_dispatch_weight(host: str, port: Optional[int] = None) -> float:
    ...
    ``elmo:8765=1.0,bert.local:8766=0.4``.
...
def _smb_checkpoint_dir() -> Path | None:
    ...
    candidates = [
        ...
        Path(
            f"/run/user/{uid}/gvfs/smb-share:server=elmo,"
...
def resolve_remote_checkpoint_path(host: str, local_path: str) -> str:
    ...
    remap the training-box repo root onto ``/Users/example/workspace/...``.
    """
    raw = str(local_path)
...
def resolve_remote_workdir_path(host: str, local_path: str) -> str:
    ...
    their native trees instead of ``/home/pokebot/poke-bot-agent/...``.
    """
    raw = str(local_path)
    host_l = host.strip().lower()
