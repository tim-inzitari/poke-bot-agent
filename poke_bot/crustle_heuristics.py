"""Sparse exact-causal current-deck guide for Crustle.

The guide is bound to Cooper Kull's exact 55th-place NAIC 2026 list in the
canonical representative registry. It produces auxiliary training labels only:
``prior_logit_bias`` is an exact zero bypass and the generic guide registry
must explicitly select Crustle before target generation is enabled.
...
* Buddy-Buddy Poffin and Ultra Ball fill an exact visible Dwebble/Crustle gap;
...
Opening Active choice, Energy attachment, Hilda, Petrel, attack choice,
healing, Hammer, gust, stadium, retreat, disruption, prizes, matchup plans,
and every hidden- or future-dependent choice are unsupported. Unsupported,
ambiguous, malformed, partial, or low-margin stages return ``None``. Missing
...
SEARCHABLE_CARD_IDS = {
    BUDDY_BUDDY_POFFIN: frozenset({DWEBBLE}),
...
def _score_stage(
    ...
    *,
    ...
    elif effect_id == MEGA_KANGASKHAN_EX:
    ...
    for combo in action_combos:
        ...
        for raw_index in combo:
            ...
            if stage_class == "initial_setup_bench":
                if card_id == DWEBBLE:
                    delta = 3.5 if visible[DWEBBLE] < 2 else 0.3
                ...
                delta = 3.6 if visible[DWEBBLE] > 0 else 0.2
    ...
    if len(scores) < 2 or max(scores) - min(scores) < ABSTENTION_MARGIN:
        return None
    return {
