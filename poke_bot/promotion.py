...
from .eval_metrics import stratified_bootstrap_interval
...
class CheckpointIdentity:
    ...
    @classmethod
    def from_path(cls, path: str | Path) -> "CheckpointIdentity":
        resolved = Path(path).expanduser().resolve()
        if not resolved.is_file():
            raise FileNotFoundError(f"checkpoint does not exist: {resolved}")
        return cls(path=str(resolved), digest=checkpoint_digest(resolved))
...
class PromotionGateConfig:
    min_games: int = 40
    #: Kept as a CLI compatibility name; because the engine has no seed API it
    #: means minimum independent games per seat, not paired trials.
...
def next_iteration(loop_state: dict[str, Any], checkpoint_iteration: Optional[int] = None) -> int:
    """Return N+1 from completed artifacts only, never from an in-flight index."""
    completed: list[int] = []
...
def evaluate_candidate_gate(
    ...
    """Score independent candidate-vs-parent games stratified by candidate seat.

    The competition engine exposes no RNG seed control. Reusing a Python seed
    therefore does not create paired trials, so the formal gate uses a
    draw-aware stratified nonparametric bootstrap.
    """
    config.validate()
    rows = list(results)
    failures = [
    ...
    if len(complete_clusters) >= 2:
        ...
        for _ in range(config.bootstrap_resamples):
            ...
            samples.append(
                statistics.fmean(
                    statistics.fmean(by_seat[seat]) for seat in (0, 1)
                )
            )
        samples.sort()
        ...
        def _quantile(q: float) -> float:
            ...
            return (
                samples[lower_index] * (1.0 - weight)
                + samples[upper_index] * weight
            )

        center = statistics.fmean(
            statistics.fmean(strata[key]) for key in ("seat0", "seat1")
        )
        lower = _quantile(alpha)
        upper = _quantile(1.0 - alpha)
        uncertainty_method = (
            "balanced_seat_cluster_nonparametric_bootstrap"
        )
    sample_ok = (
        valid_games >= config.min_games
        and all(len(values) >= config.min_complete_pairs for values in strata.values())
    )
    passed = (
        not failures
        and sample_ok
