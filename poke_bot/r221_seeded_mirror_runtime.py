"""Static r221 fresh-process seeded mirror runtime.

This evaluator-only module is the runtime companion to the r221 BO1000
launcher.  It deliberately uses the frozen r195 direct and BeliefMCTS package
inputs, but its experimental action authority is the r221 shared-turn bridge:
...
from .r221_multi_search_turn_belief_mcts import (
...
SCHEMA = "poke_bot.alakazam_local_multi_search_turn_belief_mcts_bo1000_r221_game/v1"
STRATEGY = (
...
class R221SeededMirrorRuntimeError(R215SeededMirrorRuntimeError):
    """A static r221 worker cannot make a truthful local-only receipt."""


class R221SearchDeadline(TimeoutError):
...
def _meaningful_segment_deadline(seconds: float) -> Iterator[None]:
    ...
    if float(seconds) <= 0.0:
        raise R221SearchDeadline("r221 private segment has no residual allowance")
    if not hasattr(signal, "setitimer") or not hasattr(signal, "ITIMER_REAL"):
        raise R221SeededMirrorRuntimeError(
            "r221 worker needs a Unix monotonic segment deadline"
        )
    if signal.getitimer(signal.ITIMER_REAL) != (0.0, 0.0):
        raise R221SeededMirrorRuntimeError(
            "r221 worker inherited an unrelated real-time deadline"
        )
    prior_handler = signal.getsignal(signal.SIGALRM)

    def expire(_signum: int, _frame: Any) -> None:
        raise R221SearchDeadline("r221 meaningful search segment deadline")
...
class R221TransactionalPolicyPlanner:
    """Run native BeliefMCTS under r221 acceptance/rejection authority.

    The native policy commits public history as it searches.  We retain a
    complete pre-search snapshot until the r221 controller accepts its
    ...
    def plan_turn(self, request: R215PlanRequest) -> R215PlanResult:
        ...
        if not isinstance(raw, Mapping):
            raise R221SeededMirrorRuntimeError(
                "r221 planner needs the current raw observation"
            )
        if self.pending_snapshot is not None:
            raise R221SeededMirrorRuntimeError(
                "r221 prior planner transaction was not resolved"
            )
        ...
        common = {
            "r221_frozen_direct_preview_action": list(preview),
            "r221_frozen_direct_preview_elapsed_s": preview_elapsed,
            "r221_direct_fallback_reserve_s": reserve,
            "r221_private_search_allowance_s": private_allowance,
            "r221_search_segment_allowance_s": segment_allowance,
        }
        ...
        # fallback would otherwise create a second direct inference before r221
        ...
        return r221_plan_result_from_mcts_result(
            result,
            selected_action=selected,
            extra_diagnostics={
                **common,
                "r221_policy_clock_owner": "r221_shared_actual_turn_controller",
            },
        )
    ...
    def accept_plan(self, _plan: R215PlanResult) -> None:
        if self.pending_snapshot is None:
            raise R221SeededMirrorRuntimeError(
                "r221 accepted a planner result without a transaction"
            )
    ...
    def direct_policy(self, observation: R215Observation) -> tuple[int, ...]:
        if not isinstance(observation.raw_observation, Mapping):
            raise R221SeededMirrorRuntimeError(
                "r221 direct fallback needs a raw observation"
            )
...
def _close_actual_turn(
    controller: R221MultiSearchTurnBeliefMCTS,
...
def _row_from_decision(
    *,
    decision: Any,
    planner: R221TransactionalPolicyPlanner,
    policy: Any,
...
def _run_game(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(game, Mapping) or not isinstance(pair_seal, Mapping):
        raise R221SeededMirrorRuntimeError("r221 game request lacks a sealed pair")
    ...
    if not direct_root.is_dir() or not mcts_root.is_dir():
        raise R221SeededMirrorRuntimeError("r221 game lacks physical package roots")
    seeded_engine = _seeded_engine_identity(request)

    # Loading direct first then mcts reproduces the ordinary package import
    # order.  The sealed source snapshot overlays the r221 stack into both
    ...
    if list(direct_deck) != list(mcts_deck):
        raise R221SeededMirrorRuntimeError("r221 direct/mcts frozen deck bytes diverged")
    ...
    planner = R221TransactionalPolicyPlanner(mcts_policy)
    controller = R221MultiSearchTurnBeliefMCTS(
        planner,
        direct_policy=planner.direct_policy,
        timing=R221TimingConfig(),
        game_clock=R215GameClock(),
    )
    bridge = R221PolicyTurnBridge(controller, mcts_policy)
        ...
        while not state.done and steps < max_steps:
            ...
            if identity.seat == experimental_seat:
                ...
                if "state" not in next_holder:
                    raise R221SeededMirrorRuntimeError(
                        "r221 bridge did not dispatch its verified action"
                    )
                rows.append(
                    _row_from_decision(
                        decision=decision,
...
def run_r221_seeded_mirror_operation(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    raise R221SeededMirrorRuntimeError("unsupported r221 seeded mirror operation")
