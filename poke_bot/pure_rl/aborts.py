...
def evaluate_aborts(
    *,
    ...
    """Abort only when a raw advantage-magnitude signal is genuinely dead.

    ``advantage_mean_abs`` is the preferred explicit input. The legacy
    ``mean_advantages`` argument remains for the existing trainer call site; its
    values are treated as non-negative magnitude signals. ``BatchMetrics`` now
    supplies raw ``mean(abs(R - V_frozen))`` through that compatibility field,
    never the post-whitening signed mean.
    """
    if k < 1:
