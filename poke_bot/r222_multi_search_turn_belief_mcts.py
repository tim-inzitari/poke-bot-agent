"""Fresh r222 wrapper for pre-random-boundary local BeliefMCTS evaluation.

R222 preserves r219's shared 45-second actual-turn pool and 15-second
meaningful-segment cap, but makes the chance fallback explicit: a native
BeliefMCTS result must attest that unresolved random events stopped at the
pre-random frozen-model leaf boundary.  It never grants action authority from
simulation count alone.
"""

from __future__ import annotations

from collections.abc import Mapping, Sequence
from dataclasses import asdict
from typing import Any

from .r215_full_turn_belief_mcts import R215ActionDecision, R215PlanResult, canonical_sha256
from .r219_multi_search_turn_belief_mcts import (
    R219FiniteChanceReceipt,
    R219MultiSearchTurnBeliefMCTS,
    R219PolicyTurnBridge,
    R219TimingConfig,
    commit_verified_cached_belief_action,
    r219_observation_from_raw,
    r219_plan_result_from_mcts_result,
)


R222_SCHEMA = "poke_bot.alakazam_local_multi_search_turn_belief_mcts_bo1000_r222/v1"
R222_DEFAULT_TURN_POOL_SECONDS = 45.0
R222_DEFAULT_SEARCH_SEGMENT_SECONDS = 15.0


class R222TimingConfig(R219TimingConfig):
    """Typed r222 identity; numerical timing is deliberately unchanged."""

    @property
    def identity_sha256(self) -> str:
        return canonical_sha256({"schema": R222_SCHEMA, "timing": asdict(self)})


def _integer(value: Any, *, default: int = 0) -> int:
    try:
        return int(value)
    except (TypeError, ValueError):
        return default


def _r222_action_authority_receipt(
    diagnostics: Mapping[str, Any], selected_action: Sequence[int]
) -> tuple[bool, str]:
    """Require an explicit completed root-edge backup for every selected action."""

    receipt = diagnostics.get("selected_root_action_backup_receipt")
    fields: Mapping[str, Any]
    if isinstance(receipt, Mapping):
        fields = receipt
    else:
        fields = diagnostics
    selected = tuple(int(item) for item in selected_action)
    receipt_action = fields.get("selected_action")
    if receipt_action is not None:
        try:
            if tuple(int(item) for item in receipt_action) != selected:
                return False, "selected_root_action_receipt_action_mismatch"
        except (TypeError, ValueError):
            return False, "selected_root_action_receipt_action_malformed"
    if fields.get("selected_action_legal") is not True:
        return False, "selected_root_action_legal_receipt_missing"
    if fields.get("selected_action_fully_backed_up") is not True:
        return False, "selected_root_action_backup_receipt_missing"
    visits = _integer(fields.get("selected_action_visit_count"))
    completed = _integer(
        fields.get(
            "selected_action_completed_backups", fields.get("completed_backups")
        )
    )
    if visits < 1 or completed < 1:
        return False, "selected_root_action_has_no_completed_backup"
    # The frozen r222 BeliefMCTS emits these native diagnostics directly.  A
    # runner may derive report aliases only after this complete receipt is
    # present; it may not call an absent field a zero count.
    for required in (
        "chance_samples",
        "sampled_unforceable_chance_nodes",
        "sampled_unforceable_chance_reasons",
        "unforceable_chance_boundary_nodes",
        "unforceable_chance_boundary_leaf_evaluations",
        "unforceable_chance_boundary_reasons",
    ):
        if required not in diagnostics:
            return False, f"r222_random_safety_telemetry_missing:{required}"
    if _integer(diagnostics.get("chance_samples")) != 0:
        return False, "r222_private_random_outcome_samples_nonzero"
    if _integer(diagnostics.get("sampled_unforceable_chance_nodes")) != 0:
        return False, "r222_sampled_unforceable_random_nodes_nonzero"
    sampled_reasons = diagnostics.get("sampled_unforceable_chance_reasons")
    if not isinstance(sampled_reasons, Mapping) or any(
        _integer(value) != 0 for value in sampled_reasons.values()
    ):
        return False, "r222_sampled_unforceable_random_reasons_nonzero"
    if diagnostics.get("private_unforceable_chance_samples_prohibited") is not True:
        return False, "r222_private_random_prohibition_missing"
    if diagnostics.get("seed_hunting_or_pre_randomization_prohibited") is not True:
        return False, "r222_seed_hunting_prohibition_missing"
    return True, "selected_root_action_explicitly_backed_up"


class R222MultiSearchTurnBeliefMCTS(R219MultiSearchTurnBeliefMCTS):
    """R219 timing/cache controller with r222 receipt identity."""

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        kwargs.setdefault("timing", R222TimingConfig())
        super().__init__(*args, **kwargs)

    def finish_actual_turn(self) -> Mapping[str, Any] | None:
        base = super().finish_actual_turn()
        if base is None:
            return None
        receipt = dict(base)
        receipt.update(
            {
                "schema": R222_SCHEMA,
                "base_controller_schema": base.get("schema"),
                "unforceable_randomness_mode": "pre_random_leaf_no_private_sampling",
            }
        )
        return receipt

    def act(self, *args: Any, **kwargs: Any) -> R215ActionDecision:
        base = super().act(*args, **kwargs)
        receipt = dict(base.receipt)
        receipt.update(
            {
                "schema": R222_SCHEMA,
                "base_controller_schema": base.receipt.get("schema"),
                "unforceable_randomness_mode": "pre_random_leaf_no_private_sampling",
            }
        )
        return R215ActionDecision(
            selected_action=base.selected_action, source=base.source, receipt=receipt
        )


class R222PolicyTurnBridge(R219PolicyTurnBridge):
    """R222 name for the same charged raw-observation/cache-commit bridge."""


def r222_observation_from_raw(
    raw_observation: Mapping[str, Any], policy: Any
) -> Any:
    """Build an r222 cache key while retaining the sealed r219 key format."""

    return r219_observation_from_raw(raw_observation, policy)


def r222_plan_result_from_mcts_result(
    result: Any,
    *,
    selected_action: Sequence[int] | None = None,
    extra_diagnostics: Mapping[str, Any] | None = None,
) -> R215PlanResult:
    """Convert a native result only when its selected root action is backed up."""

    plan = r219_plan_result_from_mcts_result(
        result, selected_action=selected_action, extra_diagnostics=extra_diagnostics
    )
    diagnostics = dict(plan.diagnostics)
    accepted, reason = _r222_action_authority_receipt(
        diagnostics, plan.selected_action
    )
    diagnostics["r222_selected_root_action_authority"] = accepted
    diagnostics["r222_selected_root_action_authority_reason"] = reason
    if not accepted:
        return R215PlanResult(
            selected_action=plan.selected_action,
            sims_run=0,
            continuation=(),
            diagnostics=diagnostics,
            root_action_stable=False,
            root_stability_receipt=None,
        )
    return R215PlanResult(
        selected_action=plan.selected_action,
        sims_run=plan.sims_run,
        continuation=plan.continuation,
        diagnostics=diagnostics,
        root_action_stable=plan.root_action_stable,
        root_stability_receipt=plan.root_stability_receipt,
    )


__all__ = [
    "R222_DEFAULT_SEARCH_SEGMENT_SECONDS",
    "R222_DEFAULT_TURN_POOL_SECONDS",
    "R222_SCHEMA",
    "R222MultiSearchTurnBeliefMCTS",
    "R222PolicyTurnBridge",
    "R222TimingConfig",
    "R219FiniteChanceReceipt",
    "commit_verified_cached_belief_action",
    "r219_observation_from_raw",
    "r222_observation_from_raw",
    "r222_plan_result_from_mcts_result",
]
