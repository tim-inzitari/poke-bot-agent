...
SINCE_LAST_TURN_DIM = 8
DERIVED_INFERENCE_DIM = 16 + SINCE_LAST_TURN_DIM
DERIVED_FEATURE_NAMES = (
    ...
    "self_ko_since_last_turn",
    "self_draws_since_last_turn",
    "self_plays_since_last_turn",
    "self_attaches_since_last_turn",
    "opp_ko_since_last_turn",
    "opp_draws_since_last_turn",
    "opp_plays_since_last_turn",
    "opp_attaches_since_last_turn",
)
...
class _PlayerInferenceState:
    ...
    ko_since_last_turn: int = 0
    draws_since_last_turn: int = 0
    plays_since_last_turn: int = 0
    attaches_since_last_turn: int = 0
...
class GameEventTracker:
    ...
    def derived_features(self) -> list[float]:
        ...
        return [
            ...
            float(self_state.ko_since_last_turn),
            float(self_state.draws_since_last_turn),
            float(self_state.plays_since_last_turn),
            float(self_state.attaches_since_last_turn),
            float(opp_state.ko_since_last_turn),
            float(opp_state.draws_since_last_turn),
            float(opp_state.plays_since_last_turn),
            float(opp_state.attaches_since_last_turn),
        ]

    def _player(self, player_index: int) -> _PlayerInferenceState:
        return self.players[int(player_index)]

    def _reset_since_last_turn(self, player_index: int) -> None:
        state = self._player(player_index)
        state.ko_since_last_turn = 0
        state.draws_since_last_turn = 0
        state.plays_since_last_turn = 0
        state.attaches_since_last_turn = 0
    ...
    def _record_ko(self, player_index: int) -> None:
        self._player(player_index).ko_since_last_turn += 1

    def _record_attach(self, player_index: int, *, serial: int | None) -> None:
        self._player(player_index).attaches_since_last_turn += 1
    ...
    def _process_log(self, log: dict[str, Any]) -> None:
        ...
        if log_type == LOG_TURN_START:
            ...
            if player_index == self.your_index:
                self._reset_since_last_turn(0)
                self._reset_since_last_turn(1)
            return
        ...
        if log_type in {LOG_MOVE_CARD, LOG_MOVE_CARD_REVERSE}:
            ...
            if to_area == AREA_DISCARD and from_area in {AREA_HAND, AREA_ENERGY, AREA_ACTIVE, AREA_BENCH}:
                if from_area in {AREA_ENERGY, AREA_ACTIVE}:
                    self._record_energy_discard(player_index)
                if from_area in {AREA_ACTIVE, AREA_BENCH}:
                    self._record_ko(player_index)
