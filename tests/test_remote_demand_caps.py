...
def test_elmo_bert_default_below_max() -> None:
    assert endpoint_role("elmo") == "elmo"
    assert endpoint_role("bert.local") == "bert"
    assert endpoint_default_workers("elmo", 8765) == 20
    assert endpoint_max_workers("elmo", 8765) == 40
    assert endpoint_default_workers("bert.local", 8766) == 10
    assert endpoint_max_workers("bert.local", 8766) == 20
    assert endpoint_default_workers("elmo") < endpoint_max_workers(
        "elmo"
    )
...
def test_env_overrides_caps(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(
        ...
        "elmo:8765=16,bert.local:8766=8",
    )
    monkeypatch.setenv(
        "POKEBOT_REMOTE_MAX_WORKERS",
        "elmo:8765=40,bert.local:8766=20",
    )
    assert endpoint_default_workers("elmo", 8765) == 16
    assert endpoint_max_workers("bert.local", 8766) == 20
...
def test_scheduler_starts_at_default_not_max() -> None:
    ...
    dec = sched.decision()
    assert dec.remote_demand["elmo:8765"] == 20
    assert dec.remote_demand["bert.local:8766"] == 10
    assert dec.remote_demand["elmo:8765"] < 40
...
def test_scheduler_can_grow_toward_max() -> None:
    ...
    sched.remote_defaults = {"elmo:8765": 20, "bert.local:8766": 10}
    sched.remote_maxima = {"elmo:8765": 40, "bert.local:8766": 20}
    sched._remote_demand = {"elmo:8765": 20, "bert.local:8766": 10}
    reasons: list[str] = []
    sched._step_demand(grow=True, reasons=reasons)
    assert sched._remote_demand["elmo:8765"] > 20
    assert sched._remote_demand["elmo:8765"] <= 40
...
def test_scheduler_shrink_stops_at_default() -> None:
    sched = MidIterScheduler.from_env(baseline_workers=96)
    sched.remote_defaults = {"elmo:8765": 20}
    sched.remote_maxima = {"elmo:8765": 40}
    sched._remote_demand = {"elmo:8765": 25}
    for _ in range(8):
...
def test_max_total_exceeds_local_default() -> None:
    ...
    realistic_peak = (
        ...
        + endpoint_max_workers("elmo", 8765)  # elmo 40
...
def test_remote_floor_survives_gpu_feed_bias() -> None:
    ...
    sched = MidIterScheduler(
        ...
        remote_defaults={"elmo:8765": 20, "bert.local:8766": 10},
        remote_maxima={"elmo:8765": 40, "bert.local:8766": 20},
    )
...
def test_remote_gps_ahead_without_local_need_holds_demand(
    ...
    assert dec.remote_demand["elmo:8765"] == before["elmo:8765"]
...
def test_demand_probe_shrinks_when_grow_hurts_wave_gps(
    ...
    assert dec.remote_demand["elmo:8765"] < 25
...
def test_worse_than_best_default_forces_shrink(
    ...
    assert any("remote_shrink" in r for r in dec.reason.split("+"))
    assert dec.remote_demand["elmo:8765"] < 30
    assert sched._demand_probe.grow_ceiling == 30
