...
def test_scheduler_queue_keeps_prefetch_across_large_practice_line(monkeypatch) -> None:
    raw = "\n".join(
        [
            "[remote] elmo:8765 socket_prefetch=36 (36 workers × 1)",
            "[remote] bert:8766 socket_prefetch=16 (16 workers × 1)",
            "[pure_rl] adaptive practice weights=" + ("x" * 12_000),
            "[remote] endpoint_owned_queues "
            "depths={'elmo:8765': 372, 'bert:8766': 192} "
            "high_water={'elmo:8765': 372, 'bert:8766': 192} "
            "shared_endpoint_race=disabled",
        ]
    )
    monkeypatch.setattr("scripts.dashboard_snapshot.read_tail", lambda *_args: raw)
...
def test_scheduler_queue_does_not_reuse_prior_prefetch_generation(monkeypatch) -> None:
    ...
    raw = "\n".join(
        [
            "[remote] elmo:8765 socket_prefetch=180 (36 workers × 5)",
            "[remote] bert:8766 socket_prefetch=80 (16 workers × 5)",
            queue,
            "[pure_rl] mid_iter_rebalance=start scheduler=mid_iter",
            "[remote] elmo:8765 demand=36 slots=36/36",
            "[remote] bert:8766 demand=16 slots=16/16",
            queue,
        ]
    )
    monkeypatch.setattr("scripts.dashboard_snapshot.read_tail", lambda *_args: raw)
...
def test_scheduler_reports_producer_complete_result_spool_drain(monkeypatch) -> None:
    queue = (
        "[remote] endpoint_owned_queues "
        "depths={'elmo:8765': 372} "
        "high_water={'elmo:8765': 372} "
        "shared_endpoint_race=disabled"
    )
    raw = "\n".join(
        [
...
def test_dashboard_uses_training_environment_for_checkpoint_snapshot() -> None:
    ...
    assert (
        'REMOTE_PYTHON = "/home/pokebot/miniconda3/envs/poke-bot-agent/bin/python"'
        in server_source
    )
    assert (
        '"trainer@inzi",\n'
