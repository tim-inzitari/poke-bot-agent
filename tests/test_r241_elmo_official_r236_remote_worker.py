...
def test_r241_elmo_endpoint_is_literal_and_rejects_legacy_defaults() -> None:
    assert worker.assert_r241_elmo_endpoint("elmo:8767") == "elmo:8767"
    for endpoint in (
        "elmo:8765",
        "bert:8766",
        "bert.local:8766",
...
def test_r241_receipts_bind_host_runtime_gameplay_and_are_write_once(
    ...
    assert validated["payload"]["endpoint"]["literal"] == "elmo:8767"
...
def test_r241_container_and_systemd_templates_are_isolated_and_path_preserving() -> None:
    ...
    assert "elmo:8765" not in compose
    assert "bert:8766" not in compose
    ...
    assert "/srv/poke-bot-agent/deploy/elmo" not in unit
...
def test_r241_checkpoint_transport_round_trips_the_generic_remote_path_and_port(
    ...
    assert remote_jobs._elmo_remote_checkpoint_digest("elmo", remote_path) == digest
    assert observed == {
        "host": "elmo",
        "port": 8767,
        "path": remote_path,
...
def test_r241_serve_overlay_binds_existing_preflight_receipts_without_rewriting_them(
    ...
    overlay_payload = {
        ...
        "source_snapshot": {
            ...
            "hosts": {
                "inzi": {
                    "root": "/home/pokebot/r241-source",
                    "manifest": "/home/pokebot/r241-source/r241-source-snapshot-manifest.json",
                    "outputs_root": "/home/pokebot/poke-bot-agent/outputs",
                    "staging_receipt": "/home/pokebot/poke-bot-agent/outputs/source-staging.json",
        ...
        "baseline_payloads": {
            ...
            "hosts": {
                "inzi": {
                    "root": "/home/pokebot/r241-baselines",
                    "manifest": "/home/pokebot/r241-baselines/r241-baseline-payload-manifest.json",
                    "manifest_sha256": "sha256:" + "3" * 64,
                    "baseline_tree_sha256": "sha256:" + "4" * 64,
                    "staging_receipt": "/home/pokebot/poke-bot-agent/outputs/baseline-staging.json",
                    "staging_receipt_sha256": "sha256:" + "5" * 64,
                    "canonical_roster_receipt": (
                        "/home/pokebot/poke-bot-agent/outputs/canonical-baseline-roster.json"
                    ),
                },
                "elmo": {
                    key: baseline[key]
        ...
        "remote_collection": {
            ...
            "checkpoint_transport": {
                ...
                **worker._checkpoint_transport_identity(preflight.checkpoint_transport),
                "trainer_visible_root": "/home/pokebot/r241-checkpoint-transport",
            },
        },
        "worker_image": {
            "schema": worker.R241_ELMO_WORKER_IMAGE_SCHEMA,
        ...
        "owner_start_authorization": {
            ...
            "hosts": {
                "inzi": {"path": "/home/pokebot/poke-bot-agent/outputs/owner-start-authorization.json"},
    ...
    for name, mutate in (
        ...
        (
            ...
            lambda payload: payload["baseline_payloads"]["hosts"]["elmo"].update(
                {
                    "canonical_roster_receipt": (
                        "/srv/poke-bot-agent/outputs/state/wrong-canonical.json"
                    )
                }
            ),
        ),
