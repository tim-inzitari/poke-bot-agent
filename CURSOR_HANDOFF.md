...
- Receipts: `state/crustle_rtp_rl_cotrain_r167.json`, `state/crustle_agent_no_slowking_policy_r167.json`, `outputs/state/goalmd-loop-heartbeat-crustle-r167.json`, `state/crustle_adapter_auth_epoch_repair_r167.json`, `state/crustle_iter_00000_commit_r167.json`, `state/crustle_iter_00001_collection_r167.json`, `state/crustle_iter_00001_commit_r167.json`, `state/crustle_iter_00002_collection_r167.json`, `state/crustle_iter_00002_commit_r167.json`, `state/crustle_iter_00003_collection_r167.json`, `state/crustle_iter_00003_commit_r167.json`, `state/crustle_public_mix_refill_promote_contract_validated_r167.json`, `state/crustle_promote_contract_main_hook_repair_r167.json`, `state/crustle_promote_contract_builtins_exec_repair_r167.json`.
...
- **`builtins.exec` promote-hook repair (r167):** prior hooks still missed CPython script launch (`python path/to/train_pure_rl.py` → `builtins.exec` into `__main__`). Iter3 attempt_0001 looped `public_mix_refill` for 32 rounds, fail-closed, and quarantined (`quarantine/iter_00003/attempt_0001/`, shard digest `sha256:cfbd6493…`). Bootstrap now wraps `builtins.exec`; validated (inline+subprocess) that `_replacement_schedule_contract_from_result` becomes `replacement_schedule_contract_from_result` and dual-Marnie specialist ids normalize to `marnie-s-grimmsnarl-ex`. Receipt: `state/crustle_promote_contract_builtins_exec_repair_r167.json`.
- **Live after repair:** RL MainPID `212552` / trainer `212701` (NRestarts=2, started 2026-08-06 05:48:47 EDT) sealed **iter3** under the patched process with **zero** `public_mix_refill` (8192/360517, `baseline_failed=0`). Ordinary early-stop epoch3; adapter CUDA-cache release ok; heldout WR **0.781**. Commit `commits/iter_00003.json` (`sha256:5b8c3427…`), champion remains iter0, continuous learner `sha256:dc206caf…` carries. **Do not recollect iter3.** Live: collecting **iter4**. RTP co-train MainPID `3025385` (NRestarts=0) left alone. Keep Bert Alakazam rejoin poller bootout until a CPU/MPS selector exists.
