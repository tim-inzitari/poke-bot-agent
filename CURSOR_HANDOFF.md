...
- Receipts: `state/crustle_rtp_rl_cotrain_r167.json`, `state/crustle_agent_no_slowking_policy_r167.json`, `outputs/state/goalmd-loop-heartbeat-crustle-r167.json`, `state/crustle_adapter_auth_epoch_repair_r167.json`, `state/crustle_iter_00000_commit_r167.json`, `state/crustle_iter_00001_collection_r167.json`, `state/crustle_iter_00001_commit_r167.json`, `state/crustle_public_mix_refill_promote_contract_validated_r167.json`.
...
- **Iter1 complete (not promoted).** Promote-contract overlay validated; sealed collection 8192/402530 (`baseline_failed=0`). Ordinary+adapter ok; promotion rejected (`confidence_lower_bound_not_above_threshold`). Commit `commits/iter_00001.json` (`sha256:02e91357…`), champion remains iter0, continuous learner `sha256:4630b031…` carries. **Do not recollect iter1.** Live: collecting **iter2**. Keep Bert Alakazam rejoin poller bootout until a CPU/MPS selector exists.
