    ...
    return {
        ...
        "collection": {
            ...
            "strong_public_practice": {
                ...
                "fraction_of_public": float(args.official_collect_frac),
                "temperature": float(args.strong_public_practice_temperature),
                "sample_actions": True,
                "training_eligible": True,
                "formal_eval": False,
                "roster": _opponent_specs_contract(official_specs),
...
def _assert_seed_namespace_contract(
    *,
    ...
    """Fail closed if any training seed can equal any formal-gate seed."""
    ...
    for collect_iteration, (collect_start, collect_end) in enumerate(collect_ranges):
        for gate_iteration, (gate_start, gate_end) in enumerate(gate_ranges):
            if max(collect_start, gate_start) <= min(collect_end, gate_end):
                raise RuntimeError(
                    "training/formal-gate seed namespaces overlap: "
                    f"collect_iter={collect_iteration} [{collect_start},{collect_end}] "
                    f"gate_iter={gate_iteration} [{gate_start},{gate_end}]"
                )
    return {
    ...
    for game_i in range(n_games):
        ...
        if game_i < n_self or not has_public_specs:
            ...
            opponent_archetype = str(
                (priority_archetypes or {}).get(str(spec.id)) or ""
            )
            base_jobs.append(
                {
                    **common,
                    ...
                    **(
                    ...
                    "target_provenance": {
                        ...
                        "soft_policy_targets": False,
                        "collect": (
                            str(priority_group) if is_priority else "public_mix"
                        ),
                        ...
                        **(
                        ...
                        # Keep package identity separate from the canonical
                        # matchup label consumed by the auxiliary head.
                        "opponent_id": str(spec.id),
...
def run_full_loop(args: argparse.Namespace) -> int:
    ...
    if active_gate is not None:
        ...
        practice_specs = list(official_specs)
        practice_archetypes = {}
        practice_skill_weights = {
            str(spec.id): 1.0 for spec in practice_specs
        }
    # Gate packages are supplied only through the dedicated practice schedule.
    # Exclude them (and content aliases) from the generic diverse pool so they
    # cannot be accidentally double-weighted. Formal jobs remain seed-disjoint,
    # greedy, training-ineligible, and absent from replay.
        ...
        def _kick_collect(it: int, champion: Path, dig: str) -> dict[str, Any]:
            ...
            if active_gate is not None and float(args.official_collect_frac) > 0.0:
                practice_plan = _assert_strong_public_practice_jobs(
                    ...
                    active_gate=active_gate,
                    expected_practice_games=int(
                        round(len(base_jobs) * float(args.official_collect_frac))
                    ),
                    iteration=int(it),
                    root_seed=int(args.seed),
                    formal_games=int(args.heldout_games),
