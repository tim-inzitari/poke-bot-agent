...
def expand_action_in_search(
    ...
    try:
        for _ in range(max(1, depth)):
            ...
            if int(current.get("result", -1)) >= 0:
                return evaluate_search_leaf(runtime, session, leaf_obs, root_your_index)
        ...
        return evaluate_search_leaf(runtime, session, observation_to_dict(current_state.observation), root_your_index)
    finally:
        search_end()
