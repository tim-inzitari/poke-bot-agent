...
def is_cabt_self_play_corpus(rows: list[dict[str, Any]]) -> bool:
    """True when every game originates from CABT self-play, not ladder/replay data.

    Archetype bootstrap corpora are generated by ``scripts/generate_cabt_data.py``
    (source ``multideck-cabt``) or self-play, so the top-of-ladder requirement —
    which guards the merged competition corpus — does not apply to them.
    """
    starts = episode_start_rows(rows)
    if not starts:
        return False
    for row in starts:
        ...
        if not source or is_top_of_ladder_source(source):
            return False
    return True
...
def assert_bootstrap_training_data(
    config: dict[str, Any],
    rows: list[dict[str, Any]],
    *,
    data_path: Path | str | None = None,
) -> None:
    """All data-quality gates for the training corpus, in one place.

    Consolidates the matchup-diversity, submission-separation, deck-metadata, and
    top-of-ladder checks that previously lived inline in
    ``dataset.prepare_training_tensors``. Each gate is opt-in via its config flag.
    The ladder gate auto-bypasses for CABT self-play corpora (archetype bootstraps).
    """
    label = Path(data_path).name if data_path else "training data"
    ...
    if config.get("require_top_of_ladder_data", False):
        if is_cabt_self_play_corpus(rows):
            # Archetype bootstrap / self-play corpora have no ladder games by design;
            # the ladder gate guards the merged competition corpus only.
            print(
                f"top-of-ladder gate auto-bypassed: CABT self-play corpus ({label}) "
                "carries no episodes-index games"
            )
        else:
