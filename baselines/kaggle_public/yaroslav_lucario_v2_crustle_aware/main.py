...
class LucarioPolicy:
    def __init__(self, obs: Observation):
        ...
        self.facing_crustle = any(
            pokemon is not None and pokemon.id == C.CRUSTLE
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            if self.facing_crustle and energy_count < 3:
                score += 900
    ...
    def _score_gravity_mountain(self) -> float:
        ...
        if opponent_has_stage2:
            return 3500
        if self.facing_crustle and self.stadium_id:
            return 3400
    ...
    def _score_evolve(self, option) -> float:
        ...
        if not isinstance(pokemon, Pokemon):
            return 0
        if pokemon.id == C.MAKUHITA and plan.target == 0 and not self.facing_crustle:
            return -1
        if pokemon.id == C.MAKUHITA and self.facing_crustle:
            return 9600 + len(pokemon.energies)
...
def _legal_fallback(select) -> list[int]:
    ...
    if n == 0 or select.maxCount <= 0:
        return []
    k = min(max(max(select.minCount, 0), 1), select.maxCount, n)
    return list(range(k))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        ...
        if (
            ...
            and obs.current.turnActionCount >= MAX_TURN_ACTIONS
        ):
        ...
        n = len(select.option)
        ordered = [i for i in ordered if isinstance(i, int) and 0 <= i < n]
        dedup = []
        seen = set()
