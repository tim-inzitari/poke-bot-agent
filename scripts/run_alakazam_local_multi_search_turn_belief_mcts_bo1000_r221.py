#!/usr/bin/env python3
"""Prepare or run the sealed local r221 BeliefMCTS mirror.

The only executable evaluation path is a fresh child process rooted at an
immutable r221 source snapshot.  Both arms use the frozen r195 NO-RTP model,
deck, and Matchup Adapter tree; the experimental arm uses the static r221
...
SCHEMA = "poke_bot.alakazam_local_multi_search_turn_belief_mcts_bo1000_r221/v1"
GAME_SCHEMA = "poke_bot.alakazam_local_multi_search_turn_belief_mcts_bo1000_r221_game/v1"
EVALUATION_ID = "alakazam-r221-local-multi-search-turn-belief-mcts-bo1000"
CANARY_EVALUATION_ID = "alakazam-r221-local-multi-search-turn-belief-mcts-canary10"
...
R221_CONTRACT_SHA256 = (
    "sha256:0ba3e67de761eae8c189cf4bf9900ff01574b54941ca42d0dbdc2b9fdb134f3e"
)
R221_CONTRACT_RELATIVE = Path(
    "state/alakazam-local-multi-search-turn-belief-mcts-bo1000-r221.json"
)
RUNNER_RELATIVE = Path(
    "scripts/run_alakazam_local_multi_search_turn_belief_mcts_bo1000_r221.py"
)
SOURCE_MANIFEST_NAME = "r221-source-manifest.json"


class R221RunnerError(RuntimeError):
    """The local-only r221 evaluation boundary is incomplete or unsafe."""
...
def _read_json(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise R221RunnerError(f"cannot read {label}: {path}") from exc
    if not isinstance(payload, dict):
        raise R221RunnerError(f"{label} must be a JSON object: {path}")
    return payload
...
def _physical_directory(path: Path, *, label: str) -> Path:
    try:
        ...
        raise R221RunnerError(f"cannot stat {label}: {path}") from exc
    if path.is_symlink() or not path.is_dir():
        raise R221RunnerError(f"{label} must be a physical directory: {path}")
    return path.resolve()
...
def _physical_file(path: Path, *, label: str) -> Path:
    ...
    if path.is_symlink() or not path.is_file():
        raise R221RunnerError(f"{label} must be a physical regular file: {path}")
    return path.resolve()
...
def _deck_cards_sha256(path: Path) -> str:
    ...
    try:
        ...
        raise R221RunnerError(f"cannot read frozen r195 deck: {path}") from exc
    for source in lines:
        line = source.strip()
        ...
        try:
            ...
            raise R221RunnerError("frozen r195 deck contains a noninteger card") from exc
    if len(cards) != 60:
        raise R221RunnerError(f"frozen r195 deck must have 60 cards, got {len(cards)}")
...
def _validate_source(args: argparse.Namespace) -> dict[str, Any]:
    root = _physical_directory(args.source_root, label="r221 source root")
    runner = _physical_file(root / RUNNER_RELATIVE, label="canonical r221 runner")
    if runner != Path(__file__).resolve() and not args.worker:
        ...
        raise R221RunnerError(
            "parent must execute the canonical runner physically inside --source-root"
        )
    manifest_path = _physical_file(root / SOURCE_MANIFEST_NAME, label="r221 source manifest")
    manifest = _read_json(manifest_path, label="r221 source manifest")
    if (
        manifest.get("schema")
        != "poke_bot.alakazam_local_multi_search_turn_belief_mcts_bo1000_r221_source_snapshot/v1"
        ...
        raise R221RunnerError("r221 source manifest identity is not sealed revision 221")
    ...
    if not isinstance(source_identity, str) or not re.fullmatch(
        ...
        raise R221RunnerError("r221 source manifest has no valid source-tree digest")
    expected_files = {
        root / "poke_bot/belief_mcts.py": BELIEF_MCTS_SHA256,
        root / "poke_bot/r215_full_turn_belief_mcts.py": R215_CONTROLLER_SHA256,
        root / "poke_bot/r221_multi_search_turn_belief_mcts.py": R221_CONTROLLER_SHA256,
        root / R221_CONTRACT_RELATIVE: R221_CONTRACT_SHA256,
    }
    for path, expected in expected_files.items():
        _physical_file(path, label=f"r221 sealed source {path.relative_to(root)}")
        if _sha256(path) != expected:
            raise R221RunnerError(
                f"sealed r221 source hash drifted: {path.relative_to(root)}"
            )
    contract = _read_json(root / R221_CONTRACT_RELATIVE, label="r221 contract")
    if (
        contract.get("schema") != SCHEMA
        or contract.get("owner_decision_revision") != OWNER_DECISION_REVISION
    ):
        raise R221RunnerError("sealed r221 typed contract identity drifted")
    ...
    for package, name in ((direct, "direct"), (mcts, "mcts")):
        if not _inside(package, root):
            raise R221RunnerError(f"{name} package must reside inside sealed source root")
        for relative in (
            "main.py",
            "model.pt",
            "deck.csv",
        ...
        if _sha256(package / "model.pt") != R195_CHECKPOINT_SHA256:
            raise R221RunnerError(f"{name} package is not the exact frozen r195 model")
        if _sha256(package / "matchup_tree.json") != R195_MATCHUP_TREE_SHA256:
            raise R221RunnerError(f"{name} package lost the exact r195 matchup tree")
        if _sha256(package / "cg/libcg.so") != B77_ENGINE_SHA256:
            raise R221RunnerError(f"{name} package lost the exact seeded B77 engine")
        if (package / "rtp_shadow_planner.pt").exists():
            raise R221RunnerError(f"{name} package contains forbidden RTP sidecar")
        ...
        if (
            ...
            raise R221RunnerError(f"{name} package is not explicit frozen NO RTP")
    if _deck_cards_sha256(direct / "deck.csv") != _deck_cards_sha256(mcts / "deck.csv"):
        raise R221RunnerError("direct/mcts frozen deck card order diverged")
    ...
    if direct_config.get("enabled") is not False or mcts_config.get("enabled") is not True:
        raise R221RunnerError("frozen package search roles are not direct versus BeliefMCTS")
    ...
    return {
        ...
        "controller_hashes": {
            ...
            "r221_multi_search_turn": R221_CONTROLLER_SHA256,
        },
...
def _mode_values(args: argparse.Namespace) -> tuple[str, int, int, int, int]:
    if args.mode == "canary":
        if args.pair_start != 0 or args.pair_count != CANARY_PAIRS:
            raise R221RunnerError(
                "r221 canary is exactly --pair-start 0 --pair-count 5"
            )
        return CANARY_EVALUATION_ID, CANARY_PAIRS, CANARY_GAMES, 0, CANARY_PAIRS
    if args.canary_summary is None:
        raise R221RunnerError("BO1000 requires --canary-summary from a valid r221 canary")
    if args.pair_start < 0 or args.pair_count < 1 or args.pair_start + args.pair_count > TOTAL_PAIRS:
        raise R221RunnerError("BO1000 pair range must remain within 0..499")
...
def _validate_canary_summary(path: Path) -> dict[str, Any]:
    payload = _read_json(path, label="r221 canary summary")
    required = {
        "schema": SCHEMA,
    ...
    if any(payload.get(key) != value for key, value in required.items()):
        raise R221RunnerError("r221 canary summary is incomplete or not launch-valid")
    return payload
...
def _activate_worker_source(source_root: Path) -> None:
    """Make worker imports resolve only from its immutable r221 source root."""

    root = _physical_directory(source_root, label="worker r221 source root")
    expected = str(root)
    if os.environ.get("R221_SOURCE_ROOT") != expected:
        raise R221RunnerError("worker R221_SOURCE_ROOT does not bind its source root")
    if os.environ.get("PYTHONPATH") != expected:
        raise R221RunnerError("worker PYTHONPATH is not only its sealed source root")
    if os.environ.get("PYTHONDONTWRITEBYTECODE") != "1" or os.environ.get(
        "PYTHONNOUSERSITE"
    ) != "1":
        raise R221RunnerError("worker lacks sealed Python isolation flags")
    script = Path(__file__).resolve()
    if not _inside(script, root):
        raise R221RunnerError("worker runner is not physically inside sealed source root")
    stdlib_paths = [
        item
...
def _worker(args: argparse.Namespace) -> int:
    ...
    if request.get("source_root") != str(args.source_root.resolve()):
        raise R221RunnerError("worker request source root differs from process root")
    started = _utc_now()
    expected_nonce = request.get("worker_nonce")
    if not isinstance(expected_nonce, str) or not expected_nonce.startswith("sha256:"):
        raise R221RunnerError("worker request has no immutable child nonce")
    try:
        gpu = _worker_gpu_receipt(str(request["blackwell_gpu_uuid"]))
        import importlib

        runtime = importlib.import_module("poke_bot.r221_seeded_mirror_runtime")
        result = runtime.run_r221_seeded_mirror_operation(request)
        if not isinstance(result, dict):
            raise R221RunnerError("r221 worker runtime returned no object receipt")
...
def _child_environment(
    *, runtime: Mapping[str, Any], device: str, source_root: Path
    ...
    return [
        ...
        f"R221_SOURCE_ROOT={source_root}",
        "PYTHONNOUSERSITE=1",
...
def _game_valid(document: Mapping[str, Any]) -> tuple[bool, str | None]:
    ...
    if not isinstance(rows, list) or not isinstance(closes, list):
        return False, "missing_r221_turn_telemetry"
    row_turns = {
...
def _parent(args: argparse.Namespace) -> int:
    ...
    if output_root.exists():
        raise R221RunnerError(f"refusing to reuse evaluation output root: {output_root}")
...
if __name__ == "__main__":
    ...
    except R221RunnerError as exc:
        print(f"r221 local mirror refused: {exc}", file=sys.stderr)
        raise SystemExit(2)
