#!/usr/bin/env python3
"""Extend the corrected inactive router with six older evidence days.

The canonical v33 row shards are hard-linked and verified.  Only the six
historical archives needed to improve evidence for sparse specialist routes
are featurized.  The result remains inactive until the unchanged
precision/support audit passes and a future specialist handoff binds it to an
exact checkpoint.
"""
...
TARGET_CONTAINER = "pokebot-public-tree-calibration-v34"
ROOT = Path("/mnt/Main/main/poke-adapter-oracle-v29")
ARCHIVE_ROOT = Path("/srv/poke-bot-agent/archive/episode-days")
SOURCE_ROWS = ROOT / "output/public-matchup-tree-calibration-v33/row-shards"
OUTPUT_ROOT = ROOT / "output/public-matchup-tree-calibration-v34"
OUTPUT = "/work/output/public-matchup-tree-calibration-v34"
...
def main() -> None:
    ...
    if (OUTPUT_ROOT / "PUBLIC_MATCHUP_TREE_READY.json").exists():
        raise FileExistsError("v34 output already exists; refusing replacement")
    subprocess.run(
    ...
    log = OUTPUT_ROOT / "public-matchup-tree-calibration-v34.log"
    ...
    if receipt.get("artifact_sha256") != _sha256(artifact):
        raise RuntimeError("v34 tree checksum mismatch")
    ...
    if (
        ...
        raise RuntimeError("v34 calibration or source contract invalid")
