#!/usr/bin/env python3
"""Generate CABT rollouts in Linux while querying a Mac neural policy server."""

from __future__ import annotations

import argparse
import copy
import json
import random
import sys
import urllib.error
import urllib.request
from pathlib import Path
from typing import Any

from tqdm.auto import tqdm

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from poke_agent.features import features_from_observation
from poke_agent.game_tracker import GameEventTracker
from poke_agent.rewards import assign_episode_values, is_complete_episode
from poke_agent.rule_baselines import baseline_opponents_from_names


def read_deck(path: Path) -> list[int]:
    deck = [int(token) for token in path.read_text(encoding="utf-8").replace(",", "\n").split()]
    if len(deck) != 60:
        raise ValueError(f"{path} must contain 60 card ids, found {len(deck)}")
    return deck


def json_snapshot(value: Any) -> Any:
    return json.loads(json.dumps(value, separators=(",", ":")))


def post_json(url: str, payload: dict[str, Any], *, timeout: float = 180.0) -> dict[str, Any]:
    raw = json.dumps(payload, separators=(",", ":")).encode("utf-8")
    req = urllib.request.Request(url, data=raw, headers={"Content-Type": "application/json"}, method="POST")
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return json.loads(resp.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"policy server HTTP {exc.code}: {detail}") from exc


class RemotePolicyAgent:
    def __init__(self, policy_url: str, session_id: str, *, timeout: float = 180.0):
        self.policy_url = policy_url.rstrip("/")
        self.session_id = session_id
        self.timeout = timeout
        self.fallbacks = 0
        try:
            post_json(f"{self.policy_url}/reset", {"session_id": session_id}, timeout=timeout)
        except Exception:
            self.fallbacks += 1

    def __call__(self, obs_dict: dict[str, Any]) -> list[int]:
        try:
            response = post_json(
                f"{self.policy_url}/act",
                {"session_id": self.session_id, "observation": obs_dict},
                timeout=self.timeout,
            )
            if "action" not in response:
                raise RuntimeError(f"policy server response missing action: {response}")
            return list(response["action"])
        except Exception:
            self.fallbacks += 1
            select = obs_dict.get("select") or {}
            options = select.get("option") or []
            count = min(int(select.get("maxCount", 1)), len(options))
            min_count = min(int(select.get("minCount", count)), count)
            if count <= 0:
                return []
            # Legal fallback keeps the active simulation alive. Prefer maxCount
            # like random baseline agents; this row is still marked via fallback count.
            count = max(min_count, count)
            return random.sample(range(len(options)), count)


def mark_retained_terminal(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    for row in rows:
        row["terminal"] = False
    rows[-1]["terminal"] = True
    rows[-1]["reward"] = rows[-1].get("value", rows[-1].get("reward", 0.0))


def play_remote_game(
    *,
    episode: int,
    neural_deck: list[int],
    neural_name: str,
    baseline: Any,
    policy_url: str,
    policy_timeout: float,
    max_steps: int,
    rewards: dict[str, float],
) -> tuple[list[dict[str, Any]], int]:
    from cg.game import battle_finish, battle_select, battle_start

    neural_seat = episode % 2
    deck0 = neural_deck if neural_seat == 0 else baseline.deck
    deck1 = baseline.deck if neural_seat == 0 else neural_deck
    deck0_name = neural_name if neural_seat == 0 else baseline.name
    deck1_name = baseline.name if neural_seat == 0 else neural_name
    neural_agent = RemotePolicyAgent(policy_url, f"episode-{episode}-seat-{neural_seat}", timeout=policy_timeout)
    baseline_agent = baseline.make_agent()
    agent0 = neural_agent if neural_seat == 0 else baseline_agent
    agent1 = baseline_agent if neural_seat == 0 else neural_agent

    rows: list[dict[str, Any]] = []
    tracker = GameEventTracker()
    obs, start_data = battle_start(deck0, deck1)
    if getattr(start_data, "errorPlayer", -1) >= 0:
        raise ValueError(f"deck error type={start_data.errorType} player={start_data.errorPlayer}")
    try:
        step = 0
        truncated = False
        while obs["current"]["result"] < 0 and step < max_steps:
            select = obs.get("select") or {}
            options = select.get("option") or []
            player_index = int(obs["current"]["yourIndex"])
            action = agent0(obs) if player_index == 0 else agent1(obs)
            next_obs = battle_select(action)
            terminal = int((next_obs.get("current") or {}).get("result", -1)) >= 0
            next_tracker = copy.deepcopy(tracker)
            if player_index == neural_seat:
                rows.append({
                    "episode": episode,
                    "step": step,
                    "features": features_from_observation(obs, tracker),
                    "next_features": features_from_observation(next_obs, next_tracker),
                    "observation": json_snapshot(obs),
                    "action": json_snapshot(action),
                    "next_observation": json_snapshot(next_obs),
                    "legal_action_count": len(options),
                    "select_min_count": int(select.get("minCount", 0)),
                    "select_max_count": int(select.get("maxCount", 0)),
                    "terminal": terminal,
                    "reward": 0.0,
                    "player": player_index,
                    "deck0": deck0_name,
                    "deck1": deck1_name,
                    "deck0_cards": list(deck0),
                    "deck1_cards": list(deck1),
                    "opponent_baseline": baseline.name,
                    "opponent_family": baseline.family,
                    "opponent_source": baseline.source,
                    "policy_fallbacks_so_far": neural_agent.fallbacks,
                })
            obs = next_obs
            step += 1
        if obs["current"]["result"] < 0:
            truncated = True
        result = int(obs["current"]["result"])
        if truncated or not is_complete_episode(result, terminal_obs=obs, truncated=truncated):
            return [], result
        assign_episode_values(
            rows,
            result,
            terminal_obs=obs,
            value_win=float(rewards.get("value_win", 1.0)),
            value_not_win=float(rewards.get("value_not_win", -1.0)),
            value_timeout=float(rewards.get("value_timeout", -2.0)),
        )
        for row in rows:
            row["complete"] = True
            row["truncated"] = False
        mark_retained_terminal(rows)
        return rows, result
    finally:
        battle_finish()


def main() -> None:
    parser = argparse.ArgumentParser(description="Docker/Linux CABT rollouts vs public baselines using Mac neural policy.")
    parser.add_argument("--games", type=int, default=20)
    parser.add_argument("--episode-offset", type=int, default=0)
    parser.add_argument("--out", type=Path, default=ROOT / "outputs/rollouts/remote_public_baseline_rollouts.jsonl")
    parser.add_argument("--deck", type=Path, default=ROOT / "decks/submission.csv")
    parser.add_argument("--policy-url", default="http://host.docker.internal:8765")
    parser.add_argument("--policy-timeout", type=float, default=180.0)
    parser.add_argument("--baselines", default="public")
    parser.add_argument("--append", action="store_true")
    parser.add_argument("--max-steps", type=int, default=400)
    parser.add_argument("--summary-out", type=Path, default=None)
    args = parser.parse_args()

    neural_deck = read_deck(args.deck)
    baselines = baseline_opponents_from_names(args.baselines, root=ROOT)
    args.out.parent.mkdir(parents=True, exist_ok=True)

    wins = losses = draws = errors = rows_written = 0
    mode = "a" if args.append else "w"
    with args.out.open(mode, encoding="utf-8") as handle:
        progress = tqdm(
            range(args.games),
            desc="active CABT simulations vs baselines",
            unit="game",
            dynamic_ncols=True,
            leave=True,
            mininterval=0.5,
        )
        for local_index in progress:
            episode = args.episode_offset + local_index
            baseline = baselines[episode % len(baselines)]
            try:
                rows, result = play_remote_game(
                    episode=episode,
                    neural_deck=neural_deck,
                    neural_name=args.deck.stem,
                    baseline=baseline,
                    policy_url=args.policy_url,
                    policy_timeout=args.policy_timeout,
                    max_steps=args.max_steps,
                    rewards={"value_win": 1.0, "value_not_win": -1.0, "value_timeout": -2.0},
                )
            except KeyboardInterrupt:
                raise
            except BaseException as exc:
                errors += 1
                progress.write(f"episode {episode} vs {baseline.name}: {type(exc).__name__}: {exc}")
                continue
            neural_seat = episode % 2
            if result == 2:
                draws += 1
            elif result == neural_seat:
                wins += 1
            elif result >= 0:
                losses += 1
            for row in rows:
                handle.write(json.dumps(row, separators=(",", ":")) + "\n")
            handle.flush()
            rows_written += len(rows)
            decided = wins + losses
            progress.set_postfix(
                baseline=baseline.name,
                rows=rows_written,
                win_rate=f"{(wins / decided) if decided else 0.0:.1%}",
                errors=errors,
            )

    decided = wins + losses
    summary = {
        "games": args.games,
        "wins": wins,
        "losses": losses,
        "draws": draws,
        "errors": errors,
        "rows": rows_written,
        "win_rate": (wins / decided) if decided else 0.0,
        "out": str(args.out),
    }
    print(json.dumps(summary, indent=2, sort_keys=True))
    if args.summary_out is not None:
        args.summary_out.parent.mkdir(parents=True, exist_ok=True)
        args.summary_out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")


if __name__ == "__main__":
    main()
