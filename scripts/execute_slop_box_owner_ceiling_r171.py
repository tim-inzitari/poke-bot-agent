...
RUN_DIR = Path(
    "/home/pokebot/poke-bot-agent/outputs/bootstrap/final_format_slop_box_chao_hard_r170"
)
STATE = Path("/home/pokebot/poke-bot-agent/outputs/state")
PROTECTED = Path("/home/pokebot/poke-bot-agent/outputs/pure_rl/_protected/models")
FAMILY = "final-format-slop-box-h10-rtp-expert-bootstrap-v1"
DECK_CSV = Path(
    "/home/pokebot/poke-bot-agent/outputs/submissions/"
    "slop-box-h10-rtp-cox-chao-55188658/pinned-teal-mask-ogerpon-ex.deck.csv"
)
MATCHUP = Path(
    "/home/pokebot/poke-bot-agent/outputs/state/slowking-public-matchup-tree-v33.json"
)
EXPERT = Path(
...
GUIDE = Path(
    "/home/pokebot/poke-bot-agent/config/deck_guides/slop-box-h10-rtp-north-star-v1.yaml"
)
CURRICULUM = Path(
...
RTP_LIVE = Path(
    "/home/pokebot/poke-bot-agent/outputs/rtp_fleet/slop-box-h10.live/"
...
def latest_train_acc() -> tuple[int | None, float | None]:
    log = Path(
        "/home/pokebot/poke-bot-agent/outputs/final_format_slop_box_chao_hard_r170/"
        "logs/chao_hard.log"
    )
    if not log.is_file():
        return None, None
...
def run_held_select() -> dict[str, Any]:
    ...
    cmd = [
        ...
        "--cpu-pack-root",
        "/home/pokebot/poke-bot-agent/outputs/bootstrap/cpu-packs/"
...
def queue_kaggle(manifest: dict[str, Any], reps: Path) -> dict[str, Any]:
    ...
    bundle = build_submission_bundle(
        repo_root=Path("/home/pokebot/poke-bot-agent"),
        frozen_manifest={
...
def write_register_and_rl_units() -> None:
    runtime_root = Path(
        "/home/pokebot/poke-bot-agent/outputs/final_format_slop_box_h10_rtp/runtime"
    )
    runtime_root.mkdir(parents=True, exist_ok=True)
    logs = Path("/home/pokebot/poke-bot-agent/outputs/final_format_slop_box_h10_rtp/logs")
...
ConditionPathExists=/home/pokebot/poke-bot-agent/outputs/state/final-format-slop-box-h10-rtp-bootstrap-ready.json
OnSuccess={RL_UNIT}
...
{rtp_env}ExecStartPre=/usr/bin/test -s /home/pokebot/poke-bot-agent/outputs/state/TRAINING_ARMED
ExecStartPre={PY} -u /home/pokebot/poke-bot-agent/scripts/launch_active_specialist.py --registry {registry} --check
ExecStart={PY} -u /home/pokebot/poke-bot-agent/scripts/launch_active_specialist.py --registry {registry}
Restart=on-failure
RestartSec=60
TimeoutStopSec=45
