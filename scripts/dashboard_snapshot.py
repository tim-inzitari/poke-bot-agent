...
def _selected_specialist_runtime_root(
    selector: Path = Path("/home/pokebot/.config/pokebot/specialist_runtime.env"),
    fallback: Path = Path(
...
EXPERT20_ELMO_DAILY_STATUS_GLOB = (
    "/srv/poke-bot-agent/archive/expert-latest20-derived/"
...
PROTECTED_BASELINE_GATE = Path(
    "/home/pokebot/poke-bot-model-registry/alakazam_baseline_gate/manifest.json"
)
...
def scheduler_queue_state(
    ...
    *,
    ...
    def host_key(endpoint: str) -> str:
        ...
        if "bert" in lowered or "bert" in lowered:
            return "bert"
        if "elmo" in lowered or "elmo" in lowered:
            return "elmo"
        return endpoint
...
def _managed_process_cwd(pid: int) -> Path | None:
    ...
    permitted_roots = (
        ROOT,
        Path("/home/pokebot/poke-bot-agent-deployments"),
    )
...
def matchup_pipeline_state() -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(payload, dict):
            ...
            fit_root = (
                Path("/home/pokebot/poke-bot-agent/outputs/matchup_adapters")
            ...
            log_path = Path(
                "/home/pokebot/poke-bot-agent/outputs/logs/matchup-adapter-v31.log"
            )
            latest_line = ""
            ...
            if fleet_fresh:
                ...
                if workers:
                    ...
                    merged_final = (
                        Path(
                            "/home/pokebot/poke-bot-agent/outputs/"
...
def latest10_state() -> dict[str, Any]:
    ...
    if bootstrap["active"]:
        ...
        elif "rsync" in last_run and re.search(r"bert:\S+/\s+/home/pokebot/", last_run):
...
def specialist_protocol_state(
    ...
    *,
    ...
    if runtime_identity_reconciled and runtime_run_name:
        runtime_path = (
            Path("/home/pokebot/poke-bot-agent/outputs/pure_rl")
    ...
    selector_env_path = Path("/home/pokebot/.config/pokebot/specialist_runtime.env")
