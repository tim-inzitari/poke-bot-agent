...
LOCK = Path("/home/pokebot/.local/state/pokebot/production-supervisor.lock")
MIN_FREE_GIB = 120.0
...
def main() -> int:
    ...
    with LOCK.open("a+", encoding="utf-8") as lock_handle:
        ...
        payload = {
            ...
            "units": units,
            "health": {
                "elmo_worker": tcp_endpoint_ok("elmo", 8765),
                "bert_worker": tcp_endpoint_ok("bert", 8766),
                "bert_dashboard": endpoint_ok("http://bert:8780/"),
            },
            "actions": actions,
        }
