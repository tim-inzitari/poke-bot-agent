...
ELMO_HOST="${ELMO_SSH:-root@192.168.1.143}"
BERT_HOST="${BERT_SSH:-tsinzitari@bert.local}"
BERT_REPO="${BERT_REPO:-/Users/tsinzitari/workspace/poke-bot-agent}"
...
cleanup_transaction() {
  ...
  if [[ "$rc" -ne 0 && "$DEPLOY_COMMITTED" -ne 1 ]]; then
    ...
    if [[ "$ELMO_ACTIVATION_ATTEMPTED" -eq 1 && -n "$ELMO_PREVIOUS_DIGEST" ]]; then
      wait_for_digest "192.168.1.143:8765" "$ELMO_PREVIOUS_DIGEST"
      elmo_verify_rc=$?
    else
      elmo_verify_rc=0
    fi
...
ELMO_PREVIOUS_DIGEST="$(current_remote_digest '192.168.1.143:8765')"
...
c = RemoteJobClient(*parse_endpoint("192.168.1.143:8765"), timeout_s=10)
info = c.connect()
health = c.health()
c.close()
ok = (
